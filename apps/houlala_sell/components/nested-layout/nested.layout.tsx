...
import { InnerLayout, Row, BottomBar, HoulalaSpinner } from "ui";
...
const NestedLayout: React.FC<Props> = ({ children }) => {
  ...
  const { location, error, isLoading } = useLocation(
    `${process.env.NEXT_PUBLIC_LOCATION_URL}/users/${userId}`
  );
  ...
  return (
    <div>
      ...
      <BottomBar>
        <div
          style={{
            background: router.pathname === "/dashboard/[uid]" ? "orange" : "",
          }}
          onClick={() =>
        ...
        <div
          style={{
            background: router.pathname === `/orders/[uid]` ? "orange" : "",
          }}
        ...
        <div
          style={{
            background: router.pathname === `/stock/[uid]` ? "orange" : "",
          }}
        ...
        <div
          style={{
            background:
              router.pathname === `/notification/[uid]` ? "orange" : "",
          }}
          onClick={() =>
            router.push(`/notification/${location?.uniqueIdentifier}`)
          }
          className={styles.bottomItem}
        >
          <Image src={bell} alt={"icon-image"} width={18} height={18} />
        </div>
        <div onClick={openModal} className={styles.bottomItem}>
