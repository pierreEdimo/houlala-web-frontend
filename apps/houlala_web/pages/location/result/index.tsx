...
const Index: NextPage = () => {
    ...
    const {location, isLoading, isError} = useLocation(`${LOCATION_URL}/${locationId}`);
    const {products} = useProductList(`${PRODUCT_URL}/location/${location?.uniqueIdentifier}?limit=0`);
    ...
    const filteredProducts = () => {
        return products?.filter((x) => x.name.toLowerCase().includes(word!.toString().toLowerCase()))!;
    }
            ...
            <>
                {
                    filteredProducts()?.length! < 1 ?
                        <NoItems errorMessage={"Aucuns resultat n'a ete trouve pour votre recherhe"}
